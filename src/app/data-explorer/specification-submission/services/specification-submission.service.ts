...
export class SpecificationSubmissionService {
  ...
  chooseRequestType(specificationId: Uuid): Observable<SubmissionWizardDialogResponse> {
    ...
    return this.researcherRequestEndpoints.getRequestForDataSpecification(specificationId).pipe(
      ...
      // Use filter to proceed only if `requestId` is defined
      // filter((requestId) => requestId === undefined),

      // Use switchMap to wait for the completion of the first observable before moving on
      switchMap((request) => {
        const requestId = request?.id;
        if (request && requestId !== undefined) {
          // Handle the case where requestId is defined
          return of({ requestType: request.type, requestId } as SubmissionWizardDialogResponse);
        } else {
          return projects$.pipe(
            map((projects: IdNamePair[]) => {
              ...
              const data = { projects } as SubmissionWizardDialogData;
              return data;
            }),
            // Process dialog selection only after project data is processed
            switchMap((dialogData: SubmissionWizardDialogData) => {
              return this.dialogService
                ...
                .pipe(
                  filter((response) => !!response && !response.isCancelled),
                  // ForkJoin to combine responses once dialog closes
                  switchMap((response: SubmissionWizardDialogResponse) => {
                    return of(response);
                  })
                );
            })
    ...
    */
    ...
    this.researcherRequestEndpoints
      .getRequestForDataSpecification(specificationId)
      .pipe(
