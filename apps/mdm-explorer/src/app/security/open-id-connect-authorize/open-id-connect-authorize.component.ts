...
export class OpenIdConnectAuthorizeComponent implements OnInit {
      ...
      forkJoin({
        user: this.catalogueUserService.get('currentUser').pipe(
          catchError(() => {
            ...
            this.errorMessage =
              "OpenID Connect session state has not been provided, and can't authenticate current user.";
            return EMPTY;
          })
        ),
  ...
  afterUserSignedIn(user: UserDetails, sdeProviderName: string | null): void {
    ...
    const redirectUrl = this.authenticationEndpoints.getOauthAuthorizationUrl(sdeProviderName);
