...
describe('RulesService', () => {
  ...
  it('should list representations for a rule', () => {
    ...
    endpointsStub.catalogueItem.listRuleRepresentations.mockImplementationOnce((dt, id, rid) => {
      expect(dt).toBe(domainType);
      expect(id).toBe(itemId);
      expect(rid).toBe(rule.id);
      return cold('--a|', {
  ...
  it('should get a representation for a rule', () => {
    ...
    const actual$ = service.getRepresentation(domainType, itemId, ruleId, representation.id);
    expect(actual$).toBeObservable(expected$);
  });
  ...
  it('should create a representation for a rule', () => {
    ...
    endpointsStub.catalogueItem.saveRuleRepresentation.mockImplementationOnce((dt, id, rid, pl) => {
      expect(dt).toBe(domainType);
      expect(id).toBe(itemId);
      expect(rid).toBe(ruleId);
      expect(pl).toBe(payload);
  ...
  it('should delete a representation for a rule', () => {
    ...
    const expected$ = cold('--a|', { a: true });
    const actual$ = service.deleteRepresentation(domainType, itemId, ruleId, representationId);
    expect(actual$).toBeObservable(expected$);
  });
});
