...
} from '@maurodatamapper/mdm-explorer/app/mauro/summary-metadata.service';
...
export class SummaryMetadataReportComponent implements OnInit {
  ...
  constructor(
    private summaryMetadataService: SummaryMetadataService,
  ...
  ngOnInit(): void {
    if (this.summaryMetadata) {
      this.summaryMetadataService
        .listReports(this.catalogueItemDomainType, this.catalogueItemId, this.summaryMetadata.id)
        .subscribe((data) => {
