...
export class DataElementInDataSpecificationComponent implements OnInit, OnDestroy {
  ...
  @Output() createDataSpecificationClicked = new EventEmitter<CreateDataSpecificationEvent>();
  @Output() dataSpecificationAddDelete = new EventEmitter<DataSpecificationElementAddDeleteEvent>();
        ...
        .pipe(
          ...
          switchMap((transposedDataElements) => {
            if (event.checked) {
              ...
              datamodelSubsetPayload.additions = transposedDataElements.map((de) => de.id);
            } else {
              // Do a subset remove for this data element in the data
              // specification
              datamodelSubsetPayload.deletions = transposedDataElements.map((de) => de.id);
            }
  ...
  refreshDataSpecificationMenuItems(): void {
    ...
    const idsOfDataSpecificationsContainingElement =
      this.dataElements?.length === 1
        ...
        : this.sourceTargetIntersections.sourceTargetIntersections
            .filter((sti) => this.dataElements?.every((de) => sti.intersects.includes(de.id))) // eslint-disable-line @typescript-eslint/no-non-null-assertion
            .map((sti) => sti.targetDataModelId);
