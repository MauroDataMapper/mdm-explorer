...
import { ShareDataSpecificationDialogInputOutput } from '@maurodatamapper/mdm-explorer/app/data-explorer/share-data-specification-dialog/share-data-specification-dialog.component';
import { DataModelService } from '@maurodatamapper/mdm-explorer/app/mauro/data-model.service';
import { SecurityService } from '../../security/security.service';
import { StateRouterService } from '@maurodatamapper/mdm-explorer/app/core/state-router.service';
import { FolderService } from '@maurodatamapper/mdm-explorer/app/mauro/folder.service';
import { VersionTreeSortingService } from '@maurodatamapper/mdm-explorer/app/data-explorer/version-tree-sorting.service';
import { SpecificationSubmissionService } from '@maurodatamapper/mdm-explorer/app/data-explorer/specification-submission/services/specification-submission.service';
import { SubmissionSDEService } from '@maurodatamapper/mdm-explorer/app/data-explorer/specification-submission/services/submission.sde.service';
import { DataSpecificationResearchPluginService } from '@maurodatamapper/mdm-explorer/app/mauro/data-specification-research-plugin.service';
import {
...
import { SubmissionType } from '@maurodatamapper/mdm-explorer/app/data-explorer/specification-submission/type-declarations/submission.resource';
import { SubmissionWizardDialogResponse } from '@maurodatamapper/mdm-explorer/app/data-explorer/specification-submission/specification-submission-wizard/specification-submission-wizard.component';
...
export class MyDataSpecificationDetailComponent implements OnInit, OnDestroy {
    ...
    this.okCancelItemList(itemList)
      ...
      .pipe(
        ...
        switchMap(([okCancelDialogResponse, deletedElements]) => {
          return this.removeSelectedClasses(okCancelDialogResponse, deletedElements, classList);
        }),
  ...
  shareDataSpecification() {
    ...
    this.dataSpecificationService
      ...
      .pipe(
        ...
        switchMap(
          (response: ShareDataSpecificationDialogInputOutput): Observable<DataModelDetail> => {
  ...
  private updateAllElementsSelected() {
    if (!this.allElements) {
      this.allElements = this.dataSchemaService.reduceDataElementsFromSchemas(this.dataSchemas);
    }

    this.allElementsSelected = this.allElements.every((dataElement) => dataElement.isSelected);
  }
  ...
  private updateAnyElementsSelected() {
    ...
    this.anyElementSelected = this.allElements.some((dataElement) => dataElement.isSelected);
  }
  ...
  private setDataSpecification(
    ...
    [DataSchema[]?, DataSpecificationSourceTargetIntersections?, SimpleModelVersionTree[]?]
  > {
    ...
    this.currentUserOwnsDataSpec =
      this.dataSpecification.author.replace('\\s', '') === currentUserFullName.replace('\\s', '');
  ...
  private setDataSchemasIntersectionsAndVersionTree(
    ...
    this.newerVersionExists = versionTree.findIndex((node) => node.branch === 'main') >= 0;
    ...
    this.currentVersionIsLatest = orderedTree.slice(-1)[0]?.id === this.dataSpecification?.id;
  }
  ...
  private okCancelItem(userFacingText: UserFacingText): MatDialogRef<OkCancelDialogData> {
    return this.okCancel(userFacingText.confirmationHeading, userFacingText.confirmationMessage);
  }

  private okCancelItemList(itemList: DataElementSearchResult[]): MatDialogRef<OkCancelDialogData> {
    return this.okCancel(
  ...
  private getSelectedClasses(): DataClass[] {
    return this.dataSchemaService
      ...
      .filter(
        (dataClassWithElements) =>
          dataClassWithElements.dataElements.filter((dataElement) => dataElement.isSelected)
  ...
  private removeSelectedClasses(
    ...
    [OkCancelDialogResponse, DataElementMultipleOperationResult, DataElementMultipleOperationResult]
  > {
    if (okCancelDialogResponse.result) {
      return forkJoin([
        of(okCancelDialogResponse),
  ...
  private deleteDataClass(dataClass: DataClass | undefined, dataSchema: DataSchema | undefined) {
  ...
  private deleteDataElement(
    ...
    if (dataClassWithElements?.dataElements && dataClassWithElements?.dataElements.length === 1) {
  ...
  private deleteItem(event: DataItemDeleteEvent) {
    ...
    if (itemType === 'dataClass') {
      return this.deleteDataClass(event.dataClassWithElements?.dataClass, event.dataSchema);
    }
