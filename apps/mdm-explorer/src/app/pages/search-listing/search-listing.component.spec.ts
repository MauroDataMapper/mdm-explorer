...
import { BookmarkService } from '@maurodatamapper/mdm-explorer/app/data-explorer/bookmark.service';
import { DataClassIdentifier } from '@maurodatamapper/mdm-explorer/app/mauro/mauro.types';
...
import { DataElementSearchService } from '@maurodatamapper/mdm-explorer/app/data-explorer/data-element-search.service';
import {
  DataElementSearchParameters,
  DataElementSearchResult,
  DataElementSearchResultSet,
  mapSearchParametersToParams,
} from '@maurodatamapper/mdm-explorer/app/data-explorer/data-explorer.types';
import { createBookmarkServiceStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/bookmark.stub';
import { createDataElementSearchServiceStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/data-element-search.stub';
import { createDataModelServiceStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/data-model.stub';
import { createStateRouterStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/state-router.stub';
import { createToastrServiceStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/toastr.stub';
...
import { DataElementBookmarkEvent } from '@maurodatamapper/mdm-explorer/app/data-explorer/data-explorer.types';
import { createDataSpecificationServiceStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/data-specifications.stub';
import {
  DataSpecificationService,
  DataSpecificationSourceTargetIntersections,
} from '@maurodatamapper/mdm-explorer/app/data-explorer/data-specification.service';
import { DataExplorerService } from '@maurodatamapper/mdm-explorer/app/data-explorer/data-explorer.service';
import { createDataExplorerServiceStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/data-explorer.stub';
import { SelectionService } from '@maurodatamapper/mdm-explorer/app/data-explorer/selection.service';
...
describe('SearchListingComponent', () => {
  ...
  describe('initialisation from browsing', () => {
    ...
    it('should be in ready state once initialised with results sorted by the default sortBy option', fakeAsync(() => {
      ...
      dataExplorerStub.getRootDataModel.mockImplementationOnce(() => of(mockedRootDataModel));
  ...
  describe('initialisation from search', () => {
    ...
    describe('with successful search', () => {
      ...
      it('should respond to selection changes', fakeAsync(() => {
        ...
        harness.component.ngOnInit();
        tick();
        expect(harness.component.resultSet?.items.filter((i) => i.isSelected)).toHaveLength(0);

        selectionService.add([searchResults.items[0]]);

        expect(harness.component.resultSet?.items.filter((i) => i.isSelected)).toHaveLength(1);
        expect(
          harness.component.resultSet?.items.find((i) => i.id === searchResults.items[0].id)
            ?.isSelected
        ).toBe(true);
      ...
      it('should update selection service after de-select entire page', fakeAsync(() => {
        ...
        expect(spy).toHaveBeenCalledWith(harness.component.resultSet?.items.map((item) => item.id));
      }));
    });
  ...
  describe('update search', () => {
    ...
    it.each(searchTerms)('should start a new search for %p', (searchTerm) => {
      ...
      expect(stateRouterStub.navigateToKnownPath).toHaveBeenCalledWith('/search/listing', {
        search: searchTerm,
      });
    });
  });
