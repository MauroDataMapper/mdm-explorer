  ...
  updateContactInfo(payload: CatalogueUserContactPayload) {
    ...
    dialog
      ...
      .pipe(
        ...
        switchMap((user) => {
          return forkJoin([of(user), this.dataSpecification.getDataSpecificationFolder()]);
        }),
        switchMap(([user, folder]) => {
          ...
          return this.folder.update(folder.id, {
            id: folder.id,
            label: this.dataSpecification.getDataSpecificationFolderName(user.emailAddress),
          });
        })
      )
      .subscribe(() => {
