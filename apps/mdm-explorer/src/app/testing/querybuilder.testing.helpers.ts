...
export abstract class QueryBuilderTestingHelper {
  ...
  static defaultPrimitiveTypeProfileServiceName = 'QueryBuilderPrimitiveTypeProfileProviderService';
  static defaultDataModelProfileNamespace =
    'uk.ac.ox.softeng.maurodatamapper.plugins.explorer.querybuilder';
  static defaultDataModelProfileServiceName = 'QueryBuilderPrimitiveTypeProfileProviderService';
  ...
  public static expectedDataModelProfilesStubGet = (
    ...
    expect(actualCatalogueItemId).toBe(expectedCatalogueItemId);
    expect(actualProfileNamespace).toBe(QueryBuilderTestingHelper.defaultDataModelProfileNamespace);
    expect(actualProfileName).toBe(QueryBuilderTestingHelper.defaultDataModelProfileServiceName);
  };

  public static createCoreTableProfile = (dataModel: DataModel, currentValue: string): Profile => {
    return {
      sections: [
        {
  ...
  public static mockProfile(
    ...
    profilesStub.validate.mockImplementation(() => {
      return cold(dataType.domainType !== CatalogueItemDomainType.DataModel ? 'a|' : '--a|', {
        a: {
          total: 0,
          fieldTotal: 0,
          errors: [],
