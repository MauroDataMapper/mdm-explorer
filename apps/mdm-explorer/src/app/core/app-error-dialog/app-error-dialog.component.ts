...
export class AppErrorDialogComponent implements OnInit {
  ...
  subText =
    "We're sorry, but this application encountered an error which prevents it from operating.";
  suggestionText =
    'Attempt to rectify the error (or contact your administrator), then reload this page to try again.';
  buttonText = '';
  signOutUrl?: string;
  ...
  ngOnInit(): void {
    ...
    if (this.error.status === 400 || this.error.status === 401) {
      ...
      this.subText =
        'It appears that you have been automatically signed out and will now need to sign in again to continue';
      this.suggestionText = '';
      this.error.message = '';
