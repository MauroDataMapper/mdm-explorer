...
} from '@maurodatamapper/mdm-explorer/app/data-explorer/ok-cancel-dialog/ok-cancel-dialog.component';
import { createMatDialogRefStub } from '@maurodatamapper/mdm-explorer/app/testing/stubs/mat-dialog.stub';
...
describe('OkCancelDialogComponent', () => {
  ...
  it('should call dialogRef.close with true on click OK', () => {
    ...
    const okButton = dom.query(
      (el) => el.name === 'button' && el.nativeElement.innerHTML.indexOf(component.okLabel) > -1
    );
  ...
  it('should call dialogRef.close with false on click Cancel', () => {
    ...
    const dom = harness.fixture.debugElement;
    const okButton = dom.query(
      (el) => el.name === 'button' && el.nativeElement.innerHTML.indexOf(component.cancelLabel) > -1
    );
    dialogRefStub.close.mockReset();
    okButton.triggerEventHandler('click', {});
    expect(dialogRefStub.close).toBeCalledTimes(1);
