...
export class DataSchemaService {
  ...
  reduceDataClassesFromSchemas(dataSchemas: DataSchema[]): DataClassWithElements[] {
    const dataSpecificationClasses = dataSchemas.map((dataSchema) => dataSchema.dataClasses);
    return dataSpecificationClasses.reduce((accumulator, value) => accumulator.concat(value), []);
  }
  ...
  reduceDataElementsFromSchemas(dataSchemas: DataSchema[]): DataElementSearchResult[] {
    ...
    return dataSpecificationElements.reduce((accumulator, value) => accumulator.concat(value), []);
  }
  ...
  reduceDataElementsFromSchema(dataSchema: DataSchema): DataElementSearchResult[] {
    const dataSchemaElements = dataSchema.dataClasses.map((dataClass) => dataClass.dataElements);
    return dataSchemaElements.reduce((accumulator, value) => accumulator.concat(value), []);
  }
  ...
  loadDataClasses(dataSchema: DataClass): Observable<DataClassWithElements[]> {
    return this.dataModels.getDataClasses(dataSchema).pipe(
      switchMap((dataClasses: DataClass[]) => {
        return forkJoin(dataClasses.map((dataClass) => this.loadDataClassElements(dataClass)));
      })
    );
  }
