...
describe('DataExplorerService', () => {
  ...
  describe('initialise', () => {
    it('should fail if error occurred getting api properties', () => {
      apiPropertiesStub.listPublic.mockImplementationOnce(() => cold('#', null, new Error()));
  ...
  describe('get root data model', () => {
    it('should return the root data model', () => {
      ...
      researchPluginStub.rootDataModel.mockImplementationOnce(() => cold('--a|', { a: dataModel }));
  ...
  describe('profile filter fields', () => {
    const createProfileDefinition = (dataType: ProfileFieldDataType): ProfileDefinition => {
      return {
        sections: [
          {
            name: 'section1',
    ...
    it.each(supportedDataTypes)('should return profile fields with data type %p', (dataType) => {
      mockInitialiseService();
