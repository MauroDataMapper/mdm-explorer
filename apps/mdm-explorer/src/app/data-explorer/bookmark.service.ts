...
export class BookmarkService {
  constructor(private endpoints: MdmEndpointsService, private security: SecurityService) {}
   ...
   */
  public remove(bookmarks: DataElementSearchResult[]): Observable<DataElementSearchResult[]> {
    const idsToRemove = bookmarks.map((bookmark) => bookmark.id);
    return this.index().pipe(
      switchMap((userBookmarks: DataElementSearchResult[]) => {
        const filteredBookmarks = userBookmarks.filter((bm) => !idsToRemove.includes(bm.id));
   ...
   */
  private save(bookmarks: DataElementSearchResult[]): Observable<DataElementSearchResult[]> {
    const userDetails = this.security.getSignedInUser();
    if (userDetails) {
   ...
   */
  private getBookmarksFromUserPreferences(userId: string): Observable<DataElementSearchResult[]> {
    ...
    return this.endpoints.catalogueUser.userPreferences(userId).pipe(
      map((response: MdmResponse<UserPreferencesDto>) => {
        if (response.body && response.body.bookmarks) {
          response.body.bookmarks = response.body.bookmarks.map((nativeBookmark: BookmarkDto) => {
            return {
